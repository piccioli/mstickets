...
**Fase 6 (Portale cliente e rifinitura)** e **Fase 7 (Tipologia di cliente CAI)**: 612 casi di test
in totale, organizzati in 103 argomenti, ciascuno tracciato verso un test automatico realmente
esistente nel repository tramite i manifest `fase-0-1.php` (Fase 0/Fase 1), `fase-1a.php` (Fase
1A), `fase-2.php` (Fase 2), `fase-3.php` (Fase 3), `fase-4.php` (Fase 4), `fase-5.php` (Fase 5),
`fase-6.php` (Fase 6) e `fase-7.php` (Fase 7).
...
| [`01-matrice-tracciabilita.md`](./01-matrice-tracciabilita.md) | Matrice che collega ogni test numerato (F0-xx/F1-xx/F1A-xx/F2-xx/F3-xx/F4-xx/F5-xx/F6-xx/F7-xx) al relativo test automatico nel repository, derivata dai manifest `fase-0-1.php`, `fase-1a.php`, `fase-2.php`, `fase-3.php`, `fase-4.php`, `fase-5.php`, `fase-6.php` e `fase-7.php`. |
...
| [`11-registro-esiti.md`](./11-registro-esiti.md) | Registro aggregato degli esiti di tutti i 612 test e delle anomalie rilevate. |
...
- **103 argomenti** in totale (l'elenco completo, con titoli letterali e conteggio test per
...
2. Eseguire i test in ordine in `02-fase-0.md`, poi in `03-fase-1.md`, poi in `04-fase-1a.md`, poi
   ...
   `13-fase-6.md`, poi in `14-fase-7.md`, compilando per ciascun test i relativi "Campi di
   consuntivazione" (esito PASS/FAIL/BLOCKED/NOT APPLICABLE, data, tester, evidenze, eventuale
   anomalia collegata).
3. Al termine di ciascuna fase (o dell'intero ciclo), riportare l'esito aggregato di tutti i 612
