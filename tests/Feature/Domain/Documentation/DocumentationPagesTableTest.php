...
test('registers the documents and images media collections', function (): void {
    $page = DocumentationPage::create(['title' => 'Guida', 'slug' => 'guida', 'body' => 'Contenuto']);

    expect($page->getMediaCollection('documents'))->not->toBeNull()
        ->and($page->getMediaCollection('images'))->not->toBeNull();
});
