...
- `bin/load-v1-dump path/to/dump.sql` ripristina il dump in `db_legacy`. Nessun dump reale del v1 di mstickets non è ancora disponibile in ambiente locale: la verifica finora usa solo un dump sintetico minimale. `php artisan v1:inspect` (US-008) e l'ETL vero e proprio (Fase 2+) andranno eseguiti su un dump reale non appena disponibile.
- Comandi futuri (`v1:inspect`, `v1:import`, `v1:validate`) devono leggere da `db_legacy` tramite una connessione Eloquent dedicata (nuova connessione `legacy` in `config/database.php`, da aggiungere quando servirà), mai riusando la connessione di default `pgsql`.
